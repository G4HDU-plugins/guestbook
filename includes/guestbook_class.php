...
$this->sc->use_imagecode=$this->image;
    ...
    protected function pageLoad()
    {
        ...
        if ($this->action == 'cancel' || varset($_POST['guestbook_cancel'], false) ||
            varset($_POST['guestbook_delcanc'], false)) {
            $this->action = 'view';
    ...
    {
        //print_a($_POST);
        // print_a(e107::getSecureImg()->verify_code($_POST['rand_num'],$_POST['code_verify']));
        if ($this->image) {
    ...
    protected function formDecode()
    {
        ...
        if ($this->id == 0 && ($this->prefs['emailConfirmation'] == 1 || $this->prefs['modApproveGuests'] ==
            1)) {
    ...
    protected function validate()
    {
        ...
        if ($this->prefs['allowLinksPost'] == 0 && ($this->checkLinks($this->record['guestbook_name']) ||
            $this->checkLinks($this->record['guestbook_comment']) || $this->checkLinks($this->
            record['guestbook_udf1']) || $this->checkLinks($this->recordT['guestbook_udf2']) ||
            $this->checkLinks($this->record['guestbook_udf3']) || $this->checkLinks($this->
            record['Lguestbook_udf4']) || $this->checkLinks($this->record['guestbook_udf5']))) {
    ...
    {
        $args = array('data' => array('guestbook_approved' => 1), 'WHERE' =>
                "guestbook_id='{$this->id}' AND guestbook_approved=0");
    ...
    {
        $args = array('data' => array('guestbook_approved' => 0), 'WHERE' =>
                "guestbook_id='{$this->id}' AND guestbook_approved=1");
        $result = e107::getDb()->update('guestbook', $args, false);
        if ($result) {
    ...
    protected function sendEmail($theRecord)
    {
        ...
        $link = e_SELF . "?" . $theRecord['data']['guestbook_emailconfirmcode'] .
            ".confirm." . $this->id;
        ...
        $EMAIL_TEMPLATE['guestbookConfirm']['body'] = $this->tp->parseTemplate($EMAIL_TEMPLATE['guestbookConfirm']['body'], true,
            $this->sc);
        ...
        $result = $mailout->sendEmail($theRecord['data']['guestbook_email'], $theReecord['data']['guestbook_name'],
            $eml, false);
        return $result;
    }
    ...
    function secure_imagex()
    {
        ...
        //  {
        if ($this->image) {
            $retval = " <input type = 'hidden' name = 'rand_num' value = '" . $securecodeimg =
                $this->sec_img->random_number . "' >
    " . $sec_img->r_image() .
                " <br /> <input class = 'tbox' type = 'text' name = 'code_verify' id = 'code_verify' size = '15' maxlength = '20' > ";
            //  }
    ...
    protected function processDelete()
    {
        if ($this->id > 0) {
            ...
            $this->sc->datarow['guestbook_delcanc'] = '<a href="' . $linkCancel .
                '"><button type="button" class="btn btn-secondary">' . LAN_GB_DELETECANCEL .
                '</button></a>';
            $this->sc->datarow['guestbook_delok'] = '<a href="' . $link .
                '"><button type="button" class="btn btn-danger">' . LAN_GB_DELETEOK .
                '</button></a>';
    ...
    protected function viewRecord()
    {
        ...
        if ($this->id > 0) {
            ...
            if ($row) {
                $this->sc->datarow = $row;
                $this->sc->datarow['guestbook_date'] = $this->gen->convert_date($this->sc->
                    datarow['guestbook_date'], 'short');
                ...
                $this->sc->datarow['guestbook_unapprove'] = e107::url('guestbook', 'unapprove',
                    $tag);
                ...
                $retval = $this->tp->parsetemplate($this->template->VIEW($this->admin), false, $this->
                    sc);
            }
        }
        return $retval;
...
<div id='guestbookHead' >
	<form method = 'post' action = '" . $link . "' id='guestbookEdit'>
		<input type = 'hidden' name = 'guestbook-maxtime' id = 'guestbook-maxtime' value = '" .
            $this->edittime . "' />
		<input type = 'hidden' name = 'guestbook-maxlen' id = 'guestbook-maxlen' value = '" .
            $this->prefs['commentMaxLen'] . "' />";
        ...
        $this->sc->datarow['guestbook_name'] = $this->frm->text('guestbook_name', $row['guestbook_name'],
            50, $nameoptions);
        $this->sc->datarow['guestbook_email'] = $this->frm->email('guestbook_email', $row['guestbook_email'],
            100, $emailoptions);
        $this->sc->datarow['guestbook_url'] = $this->frm->url('guestbook_url', $row['guestbook_url'],
            100);
        if ($this->prefs['allowBBCode'] != 1) {
            $this->sc->datarow['guestbook_comment'] .= $this->frm->textarea('guestbook_comment',
                $row['guestbook_comment'], 8, 50, $commentoptions, 10);
        } else {
            $template = "
	<div class='btn-toolbar'>
	   ...
	   {BB=list}{BB=fontcol}{BB=fontsize}{BB=emotes}
            ...
            $this->sc->datarow['guestbook_comment'] .= $this->frm->bbarea('guestbook_comment',
                $row['guestbook_comment'], $template, '_common', 'small', $commentoptions);
        } // end if
        ...
        $this->sc->datarow['guestbook_cancel'] = '<a href="' . $linkCancel .
            '"><button type="button" class="btn btn-secondary">' . LAN_GB_DELETECANCEL .
            '</button></a>';
        $options = array('loading' => true, 'class' => 'btn_submit btn-success');
        $this->sc->datarow['guestbook_submit'] = $this->frm->button('guestbook_submit',
            'submit', 'submit', 'Save', $options);
    ...
    function listEntries()
    {
        ...
        $text .= $this->tp->parsetemplate($this->template->LIST_DETAIL_HEADER(), true, $this->
            sc);
        if (!$this->admin) {
        ...
        if (e107::getDb()->gen($qry, false)) {
            while ($this->sc->datarow = e107::getDb()->fetch()) {
                ...
                $tag['guestbook_name'] = $this->tp->html_truncate($this->sc->datarow['guestbook_name'],
                    25, '', false);
                $tag['guestbook_name'] = str_replace(' ', '', ucwords(strtolower($tag['guestbook_name'])));
                $this->sc->datarow['guestbook_comment'] = $this->tp->html_truncate($this->sc->
                    datarow['guestbook_comment'], 60, ' ...', false);
                $tag['guestbook_comment'] = str_replace(' ', '', ucwords(strtolower($tag['guestbook_comment'])));
                $url = e107::url('guestbook', 'view', $tag);
                // die($url)  ;
                $this->sc->datarow['guestbook_listdate'] = $this->gen->convert_date($this->sc->
                    datarow['guestbook_date'], 'short');
                $link = "?guestbook_id={$this->sc->datarow['guestbook_id']}&amp;guestbook_from={$this->from}&amp;guestbook_action=";
                $link = $url;
                $text .= $this->tp->parsetemplate($this->template->LIST_DETAIL_ROW($link), true,
                    $this->sc);
            }
        } else {
            $text .= $this->tp->parsetemplate($this->template->GUESTBOOKLIST_NODETAIL(), true,
                $this->sc);
        }
        $this->sc->datarow['guestbook_nextprev'] = $this->nextprev;
        $text .= $this->tp->parsetemplate($this->template->GUESTBOOK_DETAIL_FOOTER(), true,
            $this->sc);
        return $text;
    }
    /**
